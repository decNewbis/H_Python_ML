...
if __name__ == "__main__":
    ...
    for method_ in methods:
        ...
        animation_rmse.save(os.path.join(BASE_DIR + f'/method_{method_}',
                                         f'rmse_regression_(anim)(non-linear)_smpls({nsamples_}).gif'))

        # Save plot
        plt.savefig(os.path.join(BASE_DIR + f'/method_{method_}',
                                 f"rmse_regression_(non-linear)_smpls({nsamples_}).png"))
        ...
        # Save plot
        plt.savefig(os.path.join(BASE_DIR + f'/method_{method_}',
                                 f"loss_function_rmse_(non-linear)_smpls({nsamples_}).png"))
        plt.close()
